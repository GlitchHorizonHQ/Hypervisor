    ...
    public static class GameHooks
    {
        ...
        public static void DumpSwitchDiagnostics()
        {
            try
            {
                var nm = NetworkMap.instance;
                var tm = TechnicianManager.instance;
                if (nm == null) { MelonLogger.Msg("[SwitchDiag] NetworkMap is null"); return; }

                var dict = nm.switches;
                if (dict == null) { MelonLogger.Msg("[SwitchDiag] switches dict is null"); return; }

                var keys = new System.Collections.Generic.List<string>();
                foreach (var kvp in dict) keys.Add(kvp.Key);

                MelonLogger.Msg($"[SwitchDiag] --- {keys.Count} switch(es) in nm.switches ---");
                ...
                foreach (var key in keys)
                {
                    ...
                    {
                        var sw = dict[key];
                        if (sw == null) { MelonLogger.Msg($"[SwitchDiag]   key={key} => null"); continue; }
                    ...
                    {
                        MelonLogger.Msg($"[SwitchDiag]   key={key} => exception: {ex.Message}");
                    }
                }
                ...
                MelonLogger.Msg($"[SwitchDiag] --- total: {keys.Count} normal + {brokenCount} broken ---");
            }
            catch (Exception ex)
            {
                MelonLogger.Msg($"[SwitchDiag] exception: {ex.Message}");
            }
        }
    }
}
